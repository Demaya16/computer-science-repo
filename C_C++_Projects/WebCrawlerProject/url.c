...
// The input begins with http:// or https://, so it appears to be
normalized already.
return strdup(url);
}
...
if (first_slash == NULL) {
// Base URL is something like "http://example.com" without a final
slash.
const bool domain_relative = url[0] == '/';
char *out = calloc(strlen(base) + (domain_relative? 0 : 1) +
strlen(url) + 1, sizeof(char));
if (out == NULL) {
...
// Copy base up to and including the first slash (except for the
schema).
...
char *out = calloc(last_slash - base + 1 + strlen(url) + 1,
sizeof(char));
if (out == NULL) {
fprintf(stderr, "Couldn't calloc memory for normalized URL\n");
exit(22);
...
return strlen(base) < strlen(to_validate) && strncmp(base, to_validate,
url_slash - to_validate) == 0;
}
if (url_slash == NULL)
return strlen(to_validate) < strlen(base) && strncmp(base, to_validate,
base_slash - base) == 0;
