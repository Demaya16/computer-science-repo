...
#include <stdbool.h>
/**
* Normalizes a URL. The return value is a pointer to memory allocated on the heap,
so be sure to
...
*
* If `base` is "http://example.com/bar/baz" and `url` is "a/b/c", this function
will return
* "http://example.com/bar/a/b/c".
*/
char * normalizeURL(const char *base, const char *url);
/**
* Returns true if the webpage URL in the second argument is internal to the
website from the first
